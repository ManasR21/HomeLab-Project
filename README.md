...
A complete cyber security home lab project simulating real-world enterprise infrastructure, a full attack kill chain, and detection engineering with Wazuh.
...
| Aspect | Description |
|--------|-------------|
| Objective | Build realistic enterprise environment and simulate full attack chain |
| Scope | 7 machines, 4 attack phases, Wazuh-based detection |
| Attack Tools | Nmap, Hydra, Evil-WinRM, NetExec (nxc), xfreerdp, SCP |
| Defense Tools | Wazuh SIEM, Event Logs, Sysmon |
...
User Credentials: johnd@corp.project-x-dc.com / @password123!
...
User Credentials: janed@linux-client / @password123!
...
User Credentials: sec-work@sec-box / @password123!
...
User Credentials: project-x-sec-work / @password123!
...
User Credentials: email-svr@project-x-email-svr / @password123!
...
User Credentials: attacker@attacker / attacker
...
From attacker machine (10.0.0.50):

nmap -p1-1000 -sV -Pn 10.0.0.0/24

Findings: Email server (10.0.0.8) with SSH port 22 and SMTP port 25 open.
...
Successfully logged into email server as root.
...
Sent crafted email impersonating company to janed@linux-client with malicious link.

Jane entered credentials. Attacker retrieved them:
...
Successfully accessed Linux Client (10.0.0.101).

Phase 1 Summary: Attacker compromised email server via SSH brute force, then pivoted to Linux Client via phishing.
...
From attacker machine (separate terminal):

nmap -p1-1000 -sV -Pn 10.0.0.100

nmap -Pn -p5985,5986 -sV 10.0.0.101

Findings: WinRM (Windows Remote Management) ports 5985/5986 are open on Windows Client.
...
Executed password spraying with NetExec (nxc):
...
PowerShell administrator access granted to Windows Client.

Phase 2 Summary: Attacker used password spraying on WinRM service to gain administrator access to Windows Client.
...
Findings: Port 3389/tcp (Microsoft RDP) is open.

Step 3.2 - RDP Pivot to Domain Controller

xfreerdp /v:10.0.0.5 /u:Administrator /p:@Deeboodah1! /d:corp.project-x-dc.com

Result: Full RDP session to Domain Controller established.

Phase 3 Summary: Attacker used password reuse (same Administrator password on Windows Client and DC) to RDP directly into Domain Controller.
...
File successfully transferred to attacker machine.

Step 4.2 - Create Backdoor Account on Domain Controller

From DC (PowerShell as Administrator):
...
Downloaded reverse.ps1 from attacker hosted server (10.0.0.50:8000).

Saved to: C:\Users\Administrator\AppData\Local\Microsoft\Windows\reverse.ps1

Disabled Windows Security protections (for simulation purposes).
...
Task triggers daily at midnight, establishing reverse shell back to attacker.

Phase 4 Summary: Attacker exfiltrated sensitive data, created hidden domain admin account, and established persistent reverse shell via scheduled task.

---

## Detection and Investigation

Wazuh SIEM was used for detection and investigation.

Log Analysis Workflow:

1. Navigated to Wazuh Dashboard
2. Applied Discover filter with relevant timestamps
3. Searched for specific indicators across all endpoints

Detected Activities:

Phase 1 Detection - SSH Brute Force:
Wazuh alert triggered for repeated failed SSH login attempts on email server (10.0.0.8). Alert pattern showed 50+ failed attempts followed by successful login from attacker IP 10.0.0.50.

Phase 2 Detection - Lateral Movement:
WinRM authentication logs showed successful login from unusual source (10.0.0.101) to Windows Client (10.0.0.100) using Administrator account.

Phase 3 Detection - Domain Controller Access:
RDP logon event (Event ID 4624) showed successful login to DC from Windows Client (10.0.0.100).

Phase 4 Detection - Persistence:
...
1. Weak Password on Email Server
   - SSH service used weak password (root:november)
   - Found in rockyou.txt wordlist

2. Password Reuse Across Critical Systems
   - Same Administrator password used on Windows Client and Domain Controller
   - Single credential compromise led to crown jewel asset

3. No Multi-Factor Authentication (MFA)
   - SSH, WinRM, and RDP all protected only by passwords

4. Insufficient Network Segmentation
   - Attacker pivoted from email server to internal workstations to DC

5. Lack of Privileged Access Workstations (PAWs)
   - Administrator logged into non-privileged Windows Client

Remediation Recommendations:

1. Enforce Strong Password Policy
2. Implement MFA Everywhere
3. Network Segmentation with DMZ for email server
4. Deploy Privileged Access Workstations (PAWs)
5. Improve Monitoring for credential-based attacks
6. Disable WinRM if not needed

---

## MITRE ATT&CK Mapping

| Attack Phase | Technique | Tactic |
|--------------|-----------|--------|
...
This project was inspired by ProjectSecurity.

Special thanks to Grant Collins for providing the learning experience and guidance through the lab setup and attack simulation methodology.
